...
class Envato_marketplaces {
   ...
   function __construct($api_key = null,$public_url = null) {
       if ( isset($api_key) ) $this->api_key = $api_key; // allow the user to pass the API key upon instantiation
       if ( isset($public_url) ) $this->public_url = $public_url;
   }
   ...
   */
   ...
   {  
      ...
      $search_results = $this->curl($url)->item;   
      return $search_results;//$this->fetch($url, 'item');
   }
